...
class HomeController extends Controller
{
    ...
    {
        $user = $request->user();
        $showAll = $user?->is_admin && $request->string('scope')->value() === 'all';
        $search = $request->string('search')->value();

        $servers = Server::query()
            ->with(['allocation:id,bind_ip,port,ip_alias', 'user:id,name'])
            ->when(! $showAll, fn ($query) => $query->where('user_id', $user?->id))
            ->when($search !== '', function ($query) use ($search, $showAll) {
                $query->where(function ($subQuery) use ($search, $showAll) {
                    $subQuery->where('name', 'like', "%{$search}%");

                    if ($showAll) {
                        $subQuery->orWhereHas('user', fn ($userQuery) => $userQuery->where('name', 'like', "%{$search}%"));
                    }
                });
            })
            ->orderBy('name')
